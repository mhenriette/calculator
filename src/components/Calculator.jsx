...
const Calculator = () => {
    ...
    const getOperator = (operator, number) => {
        ...
        if (clicked !== undefined && secondNumber === '') {
            ...
            setSecondNumber(eval(`(${secondNumber} ${currentOperator} ${currentNumber})`))
        ...
        <div className="grid grid-cols-4 col-start-1 col-end-5 row-start-2 row-end-7 items-center">
            <div className="grid col-start-1 col-end-4 bg-gray-300">
                <div className="grid grid-cols-3 grid-rows-1 row-start-1 row-end-1">
                    ...
                    {['AC', '+/-', '%'].map((el, index) => {
                        return <Button children={el} key={index}
                            onClick={() => specialOp(el, index)} />
                    })}
                </div>
                <div className="grid grid-cols-3 grid-rows-4">
                    {["7", "8", "9", "4", "5", "6", "1", "2", "3", "0", "."].map((el, index) => { return <Button children={el} large={el === '0' ? true : false} onClick={() => getNumber(el)} key={index} className="border-gray-500" clicked={clicked} isDisabled={isDisabled} /> })}
                </div>
            </div>
            <div className="grid grid-cols-1 col-start-4 col-end-5 bg-orange-500">
                {["/", "x", "-", "+", "="].map((el, index) => {
                    return <Button children={el} key={index} onClick={() => getOperator(el, index)} clicked={clicked} index={index} className="text-2xl font-semibold"
                    />
                })}
            </div>
        </div>
